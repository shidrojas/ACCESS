    ...
    try {
      ...
      if (result.status === "success") {
        ...
        switch (result.user.role) {
          case "member":
            navigate("/mainLayout");           // Student
            break;
          case "admin":
            navigate("/adminMainLayout");      // Admin
            break;
          case "project_manager":
            navigate("/instructorMainLayout"); // Instructor
            break;
          default:
            setWarning("Unknown role. Contact admin.");
        }
      } else {
              ...
              {step === "forgot" && (
                <>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    <button className="login-button" onClick={handleForgotConfirm}>CONFIRM</button>
                    <button className="login-button" onClick={() => { clearForgotFields(); setStep("login"); }}>CANCEL</button>
                  </div>
                </>
              )}
