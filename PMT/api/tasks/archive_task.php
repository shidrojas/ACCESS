...
if (!$isHostOrCoHost) {
    ...
    echo json_encode(["status" => "error", "message" => "Only project hosts and co-hosts can archive tasks"]);
    exit;
}

// Archive the task (soft delete by setting status to 'Archived')
$sql = "UPDATE tasks SET Status = 'Archived' WHERE TaskID = $TaskID";
...
if ($conn->query($sql)) {
    ...
    $desc = "Task '$TaskName' was archived";
    ...
    echo json_encode(["status" => "error", "message" => "Failed to archive task: " . $conn->error]);
}
